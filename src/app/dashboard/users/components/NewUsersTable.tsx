...
import { Activity, CheckCircle, ChevronLeft, ChevronRight, Copy, Mail, ShieldCheck, UserPlus } from "lucide-react";
import { Badge, Button } from "@/components/ui";
import { apiClient } from "@/services/api-client";
import { EmailActivityItem, NewUser, SignupPurpose } from "../types";
import { formatDateTime, formatRelativeTime } from "../utils";
import { toast } from "sonner";
import { NewUserEmailActivity } from "./NewUserEmailActivity";
...
export function NewUsersTable({
  ...
  const [activityEmail, setActivityEmail] = React.useState<string | null>(null);
  const [activityRows, setActivityRows] = React.useState<EmailActivityItem[]>([]);
  ...
  const loadEmailActivity = async (email: string, force = false) => {
    if (activityEmail === email && !force) {
    ...
    try {
      const response = (await apiClient.get(`/admin/emails/${encodeURIComponent(email)}/activity?limit=10`)) as {
        success: boolean;
        data: EmailActivityItem[];
        error?: string;
      };
      if (!response.success) {
        setActivityError(response.error || "Unable to load email activity.");
        return;
      }
      setActivityRows(response.data || []);
    } catch (error) {
      setActivityError(error instanceof Error ? error.message : "Unable to load email activity.");
    } finally {
      setActivityLoading(false);
    }
  };

  const getPurposeLabel = (purpose: SignupPurpose) => (purpose === "task_creation" ? "business" : "perform task");
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <label className="flex items-center gap-2 text-xs font-bold text-zinc-500">
            ...
            >
              ...
              <option value="task_creation">Business</option>
              <option value="perform_tasks">Perform task</option>
            </select>
          </label>
        </div>
      </div>
              ...
              users.map((user) => {
                ...
                return (
                  <React.Fragment key={user.id}>
                    <tr className="transition-colors hover:bg-zinc-800/20">
                      ...
                      <td className="px-6 py-4">
                        <Badge variant={user.signupPurpose === "task_creation" ? "purple" : "default"}>
                          {getPurposeLabel(user.signupPurpose)}
                        </Badge>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {user.emailVerified ? (
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button
                              type="button"
                              disabled={!user.email || activityLoading}
                              onClick={() => user.email && loadEmailActivity(user.email)}
                              className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-800/40 px-3 py-1.5 text-xs font-bold text-zinc-300 transition-colors hover:bg-zinc-800 disabled:opacity-50"
                              title="Check email activity"
                            >
                              <Activity className="h-3.5 w-3.5" />
                              {activityLoading && activityEmail === user.email ? "Loading..." : "Activity"}
                            </button>
                            <button
                              type="button"
                              disabled={!user.email || isResending || isVerifying}
                    ...
                    {activityEmail === user.email && user.email ? (
                      <NewUserEmailActivity
                        email={user.email}
                        rows={activityRows}
                        loading={activityLoading}
                        error={activityError}
                        onRefresh={() => loadEmailActivity(user.email!, true)}
                      />
                    ) : null}
                  </React.Fragment>
                );
