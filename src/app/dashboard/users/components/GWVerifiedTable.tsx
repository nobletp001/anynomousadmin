...
interface GWVerifiedTableProps {
  users: User[];
    ...
    <div className="backdrop-blur-md bg-zinc-900/30 border border-zinc-800/80 rounded-2xl shadow-xl overflow-hidden">
            ...
            {users.length ? (
              users.map((user) => (
                <tr
                  ...
                  className={`transition-colors cursor-pointer ${
                    user.disabled ? "bg-red-500/5" : "hover:bg-zinc-800/20"
                  }`}
                >
                  ...
                  <td className="px-6 py-4 text-zinc-400">
                    {user.whatsappNumber ?? <span className="text-zinc-600">—</span>}
                  </td>
                  <td className="px-6 py-4">
              ...
              <tr>
                <td colSpan={7} className="px-6 py-16 text-center text-zinc-500">
                  ...
                  No Google & WhatsApp verified users found
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-zinc-800/60">
        <span className="text-xs text-zinc-550">
          Page {page} of {totalPages} · {totalUsers} total verified users
        </span>
